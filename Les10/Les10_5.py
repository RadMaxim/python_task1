...
count = 0
apple = cv2.imread("./img/c25c94fe96_1000.jpg")
...
while True:
    ...
    if hands:
        ...
        if (
            r < 20
            and x3 - SIZE_RECTANGLE // 2 < cx < x3 + SIZE_RECTANGLE // 2
            and y3 - SIZE_RECTANGLE // 2 < cy < y3 + SIZE_RECTANGLE // 2
        ):
            print("touch")
            x3 = cx
            y3 = cy
    ...
    frame[
        (y3 - SIZE_RECTANGLE // 2) : (y3 + SIZE_RECTANGLE // 2),
        (x3 - SIZE_RECTANGLE // 2) : (x3 + SIZE_RECTANGLE // 2),
    ] = apple
    img = cv2.resize(
        img,
        (
